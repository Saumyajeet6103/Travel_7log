...
function EditLegModal({ leg, onSave, onClose }: EditLegModalProps) {
  ...
  const field = (key: keyof TrainLeg, label: string, placeholder = '') => (
    <div>
      <label className="block text-xs font-medium text-[#A0AEC0] mb-1">{label}</label>
      <input
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#16213E] border border-[#0F3460] rounded-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-[#16213E] border-b border-[#0F3460] px-5 py-4 flex items-center justify-between">
          <h2 className="font-heading font-bold text-base text-[#E8F5E9]">✏️ Edit: {leg.label}</h2>
          <button onClick={onClose} className="text-[#A0AEC0] hover:text-[#E8F5E9] text-xl leading-none">×</button>
        </div>
    ...
    <div className="relative z-10 mx-auto w-full max-w-sm">
      <button
        ...
        className={`w-full border rounded-xl px-4 py-2.5 flex items-center justify-between gap-3 transition-all text-left ${
          isTBD
            ? 'bg-[#0F3460]/60 border-[#0F3460] border-dashed text-[#A0AEC0]'
            : 'bg-[#16213E] border-[#2D9CDB]/40 hover:border-[#2D9CDB]/70'
        }`}
      >
        <div className="flex items-center gap-2">
          <Train size={14} className={isTBD ? 'text-[#A0AEC0]' : 'text-[#2D9CDB]'} />
          <div>
            <p className={`text-xs font-medium ${isTBD ? 'text-[#A0AEC0]' : 'text-[#E8F5E9]'}`}>
              {leg.label}
            </p>
        ...
        <div className="flex items-center gap-2">
          {isAdmin && (
            <span
              ...
              className="p-1 rounded-lg hover:bg-[#0F3460] text-[#A0AEC0] hover:text-[#52B788] cursor-pointer"
            >
              <Pencil size={11} />
            </span>
          )}
      ...
      {open && (
        <div className="mt-1 bg-[#16213E] border border-[#0F3460] rounded-xl p-4 space-y-2 animate-fade-in">
          {[
            ...
            <div key={label} className="flex items-center gap-2">
              <Icon size={12} className="text-[#A0AEC0] flex-shrink-0" />
              <span className="text-xs text-[#A0AEC0] w-16 flex-shrink-0">{label}</span>
              <span className={`text-xs font-medium ${value === '—' ? 'text-[#A0AEC0]/50' : 'text-[#E8F5E9]'}`}>
                {value}
              </span>
            </div>
          ))}
          {leg.notes && (
            <p className="text-xs text-[#A0AEC0] italic bg-[#0F3460]/40 rounded-lg px-3 py-2 mt-2">
              💡 {leg.notes}
            </p>
          )}
        </div>
          ...
          <div key={node.id}>
            ...
            <div className={`flex items-center gap-4 relative ${i > 0 ? 'mt-0' : ''}`}>
              ...
              <div className="flex flex-col items-center flex-shrink-0 w-8">
                {i > 0 && <div className="w-0.5 h-4 bg-[#0F3460]" />}
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-lg border-2 flex-shrink-0 ${
                    node.highlight
                      ? 'bg-[#52B788]/20 border-[#52B788] shadow-[0_0_12px_rgba(82,183,136,0.3)]'
              ...
              <div className={`flex-1 py-3 ${i < NODES.length - 1 ? '' : ''}`}>
                <div className="flex items-center gap-2">
                  <p className={`font-heading font-bold text-base ${node.highlight ? 'text-[#52B788]' : 'text-[#E8F5E9]'}`}>
                    {node.label}
                  </p>
                  <span className="text-xs text-[#A0AEC0] bg-[#0F3460] px-2 py-0.5 rounded-full">{node.date}</span>
                </div>
                <p className="text-xs text-[#A0AEC0]">{node.sub}</p>
              </div>
            </div>
            ...
            {step && leg && (
              <div className="flex gap-4">
                {/* Vertical line */}
                <div className="flex flex-col items-center flex-shrink-0 w-8">
                  <div className="w-0.5 flex-1 bg-[#0F3460]" style={{ minHeight: '8px' }} />
                </div>
                {/* Leg card */}
                <div className="flex-1 py-2">
                  {step.returnLeg ? (
                    <div className="text-xs text-[#A0AEC0] italic px-2 py-1.5 bg-[#0F3460]/30 rounded-lg border border-dashed border-[#0F3460]">
